...
// Date: 2020-09-22 19:32:28 UTC
// Source: chre_api/include/chre_api/chre/wifi.h @ commit 2f242278
...
static bool chppWifiConvertScanEventToChre(const struct ChppWifiScanEvent *in,
  ...
  if (in->scannedFreqList.length != 0) {
    ...
    uint32_t *scannedFreqListIn =
        (uint32_t *)&((uint8_t *)in)[in->scannedFreqList.offset];
  ...
  if (in->results.length != 0) {
    ...
    struct ChppWifiScanResult *resultsIn =
        (struct ChppWifiScanResult *)&((uint8_t *)in)[in->results.offset];
...
static bool chppWifiConvertScanParamsToChre(const struct ChppWifiScanParams *in,
  ...
  if (in->frequencyList.length != 0) {
    ...
    uint32_t *frequencyListIn =
        (uint32_t *)&((uint8_t *)in)[in->frequencyList.offset];
  ...
  if (in->ssidList.length != 0) {
    ...
    struct ChppWifiSsidListItem *ssidListIn =
        (struct ChppWifiSsidListItem *)&((uint8_t *)in)[in->ssidList.offset];
