...
// Date: 2020-06-15 20:42:31 UTC
// Source: chre_api/include/chre_api/chre/gnss.h @ commit ea747534
...
//! chreGnssDataEvent as struct ChppGnssDataEvent
static size_t chppGnssSizeOfDataEventFromChre(
    const struct chreGnssDataEvent *dataEvent) {
  size_t encodedSize = sizeof(struct ChppGnssDataEvent);
  encodedSize +=
...
bool chppGnssDataEventFromChre(const struct chreGnssDataEvent *in,
  ...
  *out = chppMalloc(payloadSize);
  if (*out != NULL) {
    ...
    chppGnssConvertDataEventFromChre(in, *out, payload, payloadSize,
                                     &vlaOffset);
    *outSize = payloadSize;
    return true;
  }
...
bool chppGnssLocationEventFromChre(const struct chreGnssLocationEvent *in,
                                   struct ChppGnssLocationEvent **out,
                                   size_t *outSize) {
  CHPP_NOT_NULL(out);
  CHPP_NOT_NULL(outSize);

  size_t payloadSize = sizeof(struct ChppGnssLocationEvent);
  *out = chppMalloc(payloadSize);
  if (*out != NULL) {
    chppGnssConvertLocationEventFromChre(in, *out);
    *outSize = payloadSize;
    return true;
  }
  return false;
