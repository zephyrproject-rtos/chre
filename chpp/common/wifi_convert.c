...
// Date: 2021-01-07 22:40:44 UTC
// Source: chre_api/include/chre_api/chre/wifi.h @ commit 70baadf2
...
static bool chppWifiConvertRangingEventToChre(
  ...
  if (in->results.length == 0) {
    ...
    if (in->results.offset + in->results.length > inSize ||
        in->results.length !=
            in->resultCount * sizeof(struct chreWifiRangingResult)) {
      return false;
    }
  ...
  if (in->targetList.length == 0) {
    ...
    if (in->targetList.offset + in->targetList.length > inSize ||
        in->targetList.length !=
            in->targetListLen * sizeof(struct chreWifiRangingTarget)) {
      return false;
    }
