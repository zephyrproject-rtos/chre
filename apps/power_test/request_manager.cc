...
bool RequestManager::requestTimer(bool enable,
                                  TimerType type,
                                  Nanoseconds delay) {
  bool success = false;
  if (enable) {
    NestedDataPtr<TimerType> timerType(type);
    uint32_t timerId = chreTimerSet(delay.toRawNanoseconds(),
                                    timerType.dataPtr,
                                    false /* oneShot */);
  ...
  LOGI("RequestTimer success %d, enable %d, type %d, delay %" PRIu64,
       success, enable, type, delay.toRawNanoseconds());
  return success;
}
...
bool RequestManager::requestGnssLocation(
  ...
  bool success;
  if (enable) {
    success = chreGnssLocationSessionStartAsync(scanIntervalMillis,
                                                minTimeToNextFixMillis,
                                                nullptr /* cookie */);
  } else {
...
bool RequestManager::requestAudio(bool enable,
  ...
  if (enable) {
    ...
    success = chreAudioConfigureSource(0 /* handle */,
                                       false /* enable */,
                                       0 /* bufferDuration */,
                                       0 /* deliveryInterval */);
  }
  LOGI("RequestAudio success %d, enable %d, bufferDurationNs %" PRIu64,
       success, enable, bufferDurationNs);
  return success;
}

bool RequestManager::requestSensor(bool enable,
                                   uint8_t sensorType,
  ...
  if (success) {
    if (enable) {
      success = chreSensorConfigure(sensorHandle,
                                    getModeForSensorType(sensorType),
                                    samplingIntervalNs,
                                    latencyNs);
    } else {
    ...
    switch (static_cast<MessageType>(hostMessage.messageType)) {
      case MessageType::TIMER_TEST: {
        ...
        if (verifyMessage<TimerMessage>(hostMessage, &msg)) {
          success = requestTimer(msg->enable(),
                                 TimerType::WAKEUP,
      ...
      case MessageType::GNSS_LOCATION_TEST: {
        ...
        if (verifyMessage<GnssLocationMessage>(hostMessage, &msg)) {
          success = requestGnssLocation(msg->enable(),
                                        msg->scan_interval_millis(),
      ...
      case MessageType::SENSOR_REQUEST_TEST: {
        ...
        if (verifyMessage<SensorRequestMessage>(hostMessage, &msg)) {
          success = requestSensor(msg->enable(),
                                  static_cast<uint8_t>(msg->sensor()),
                                  msg->sampling_interval_ns(),
                                  msg->latency_ns());
        }
        break;
      }
