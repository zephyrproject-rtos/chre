...
//! Location fixes supplied from chreGnssConfigureLocationMonitor() include all
//! fixes generated by the GNSS engine, not just those requested by other
//! nanoapps within CHRE
#define CHRE_GNSS_CAPABILITIES_GLOBAL_MONITOR  UINT32_C(1 << 2)
 ...
 * There will be no duplication of events if a location monitor and location
 * session are active in parallel.
 *
 ...
 * return false.
 *
 * If chreGnssGetCapabilities() includes CHRE_GNSS_CAPABILITIES_GLOBAL_MONITOR,
 * location fixes supplied by this monitor registration include all fixes
 * performed by the GNSS engine, including those initiated by the applications
 * processor and any internal clients. If this flag is not set, then only fixes
 * requested by other nanoapps are included.
 *
