...
class EventLoopManager : public NonCopyable {
 ...
 private:
  ...
  // TODO: the number of event loops should be compile-time fixed and defined by
  // the platform. For now, the implementation limits us to only supporting one.
  static constexpr size_t kNumEventLoops = 1;
  ...
  static_assert(kNumEventLoops == 1, "More than 1 EventLoop requires updates");
  ...
  FixedSizeVector<EventLoop, kNumEventLoops> mEventLoops;
