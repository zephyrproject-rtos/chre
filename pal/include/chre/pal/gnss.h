...
struct chrePalGnssCallbacks {
     ...
     * system re-send requests for any active sessions and its current location
     * monitor setting. For example, if the GNSS subsystem has recovered from a
     * system crash, this function can be used to silently (from the client's
     * perspective) restore open sessions.
     */
...
struct chrePalGnssApi {
     ...
     * engine. For example, when the location monitor is enabled, the PAL
     * implementation will deliver location fixes computed as a result of
     ...
     *
     * This feature is supported when the CHRE_GNSS_CAPABILITIES_GLOBAL_MONITOR
     * bit is returned in getCapabilities(). If not supported, this function
     * must always return false.
     *
     * This configuration represents CHRE's persistent interest in receiving all
     * available GNSS fixes, independent of the state of CHRE's own location or
     * measurement sessions as set by controlLocationSession() and
     * controlMeasurementSession(). For example, toggling this setting on and
     * off must not have any impact on the behavior of an ongoing location
     * session.
     *
     * This setting must either persist across subsystem restarts of the GNSS
     * engine, or the PAL must use the requestStateResync() callback to retrieve
     * the current requested location monitor state after the GNSS engine has
     * recovered from a restart.
     *
     * @param enable true to turn the location monitor on, false to turn it off
     *
     ...
     * @since v1.2
     */
    bool (*configureLocationMonitor)(bool enable);
};
